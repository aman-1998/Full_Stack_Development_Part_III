...
public class BrandResource {
	...
	public Response getBrand(@PathParam("brandId") int brandId, @Context UriInfo uriInfo) {
		...
		String productsUri = uriInfo.getBaseUriBuilder()
									.path(BrandResource.class)  // .path(ProductResource.class)
									.path(ProductResource.class, "getProductsByBrand")
									.resolveTemplate("brandId", brandId)
									.build().toString();
		Link productsLink = new Link(productsUri, "products");
